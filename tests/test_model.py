...
def test_user():
    with tempfile.TemporaryDirectory() as workspace:
        ...
        john = db.create_user("john.appleseed@apple.com", "Setec astronomy", "John", None)
        assert john.id is not None
        db.create_user("Jane.Doe@apple.com", "too many secrets", "Jane", john.id)

        assert db.count_users() == 2, f"users = {db.count_users()}"
        db.close()
        ...
        db.change_user_info(john.id, password_hash=financial_game.model.Database.hash_password("setec astronomy"))
        ...
        assert len(db.get_user_sponsored(jane.id)) == 0
        john_sponsored = db.get_user_sponsored(john.id)
...
def test_serialize():
    with tempfile.TemporaryDirectory() as workspace:
        ...
        assert jane.sponsor_id == john.id
        assert len(db.get_user_sponsored(jane.id)) == 0
        assert len(db.get_user_sponsored(john.id)) == 1
        assert db.get_user_sponsored(john.id)[0].id == jane.id
